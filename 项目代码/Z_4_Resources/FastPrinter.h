...
FastPrinter::FastPrinter(size_t x, size_t y) :sizeX(x), sizeY(y) {//x,y数组大小，就屏幕大小？
	initDrawer();
}
...
void FastPrinter::setData(const char* _in_data, const BYTE* _in_front_color, const BYTE* _in_back_color, SMALL_RECT _area) {
	...
	SHORT row = (_area.Right - _area.Left);//_area里面有矩形的上下左右，这里得到了长
	for (WORD _i = _area.Top, i = 0; _i < _area.Bottom; _i++, i++) {
		...
		memcpy(dataGrid + dataIndex, _in_data + inDataIndex, row);//从不同的字节位数进行复制
...
void FastPrinter::_setFontSize(const WORD x) {
	...
	cfi.dwFontSize.X = 0;//0
	cfi.dwFontSize.Y = x;//字体的尺寸
...
void FastPrinter::initDrawer() {
	...
	// get font size
	CONSOLE_FONT_INFOEX cfi;//控制台字体信息
	cfi.cbSize = sizeof(cfi);
	GetCurrentConsoleFontEx(GetStdHandle(STD_OUTPUT_HANDLE), FALSE, &cfi);
	...
	SetWindowPos(hwnd, HWND_TOP, 0, 0, cfi.dwFontSize.X * sizeX, cfi.dwFontSize.Y * sizeY, 0);//0
...
void FastPrinter::_drawC() {
	...
	for (size_t i = 0; i < sizeY; i++) {
		char posSequence[16] = "\x1b[001;001H";//得到i的个十百位的字符数
